...
def Fig(tp, yn,cpr,fpr,En,Opy):
    ...
    if tp==1:
        ...
        plt.fill_between(xn[A[0]], 0, 600, facecolor='cyan', alpha=0.3, label = 'Profitable N Rate Range(98%) '+str(round(xn[Xmin],2))+'-'+str(round(xn[Xmax],2)))
        
        MRTN=xn[np.argmax(Yrtn, axis=0)]
        YRTN=max(Yrtn)
        plt.scatter(MRTN,YRTN,c='r', label = 'MRTN='+str(round(MRTN))+' Net Return to N at MRTN Rate='+str(round(YRTN,2)))
        ...
        plt.fill_between(xn[A[0]], 0, 110, facecolor='cyan', alpha=0.3, label = 'Profitable N Rate Range(98%) '+str(round(xn[Xmin],2))+'-'+str(round(xn[Xmax],2)))
        ...
        plt.figure(figsize=(15,10))
        a=pd.cut(En,[0,25,50,75,100,125,150,175,200,225,250], labels=[u"(0,25]",u"(25,50]",u"(50,75]",u"(75,100]",u"(100,125]",u"(125,150]",u"(150,175]",u"(175,200]",u"(200,225]",u"(225,250]"])
        b=a.value_counts()
        b2=100*b/sum(b)
        b3=b2.sort_index()
